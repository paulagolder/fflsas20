...
use App\Form\Type\LocationForm;

use App\Service\MyLibrary;

use App\Controller\LinkrefController;
use App\Entity\location;
use App\Service\KMLFileUploader;
...
class LocationController extends AbstractController
{
    ...
    public function Showone($lid)
    {
        ...
        if(count($childrenlist))
        {
            ...
            for($i=0; $i<$l;$i++)
            {
                ...
                $achild = $this->getDoctrine()->getRepository("App:Location")->findOne($cid);
                if($i>0) $childrenkml .= ",";
    ...
    public function upload($lid,Request $request)
    {
        try {
            ...
            $file->move ( $this->getParameter ( 'kml-folder-long'), $original_name  );
      ...
      public function LocationSearch($search, Request $request)
    {
        ...
        {
            $sfield = "%".$pfield."%";
            $locations= $this->getDoctrine()->getRepository("App:Location")->findSearch($sfield);

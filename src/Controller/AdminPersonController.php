...
class AdminPersonController extends AbstractController
{
    ...
    public function __construct( MyLibrary $mylib ,RequestStack $request_stack, LinkrefController  $linkref)
    {
        $this->mylib = $mylib;
    ...
    public function Editone($pid)
    {
        ...
        foreach($ref_ar as $key => $ref)
        {
            ...
            if($image )
            {
                ...
                $itext_ar = $this->getDoctrine()->getRepository("App:Text")->findGroup("image",$imageid);
        ...
        $linkrefs = $this->linkref->getLinks("person",$pid, $this->lang);
    ...
    {
       $lref = $this->getDoctrine()->getRepository('App:Imageref')->findMatch('person',$pid,$iid);
       if(!$lref)
       {
    ...
    public function addContent($pid,$cid)
    {
        $this->lang = $this->requestStack->getCurrentRequest()->getLocale();
        $path = "/content/".$cid;
        $lref = $this->getDoctrine()->getRepository('App:Linkref')->findbyPath('person',$pid,$path);
        if(!$lref)
        {
        $obj = $this->getDoctrine()->getRepository('App:Content')->findContentLang($cid,$this->lang);
        $label= $obj->getLabel();
        $linkref = new Linkref();
    ...
    {
        $path = "/biblo/".$bid;
        $book = $this->getDoctrine()->getRepository('App:Linkref')->findbyPath('biblo',$bid,$path);
        if(!$book)
        {
        $obj = $this->getDoctrine()->getRepository('App:Biblo')->findOne($bid);
        $label= $obj->getTitle();
        $linkref = new Linkref();
        $linkref->setLabel($label);
        $linkref->setPath($path);
    ...
    {
        $path = "/url/".$uid;
        $url = $this->getDoctrine()->getRepository('App:Linkref')->findbyPath('url',$uid,$path);
        if(!$url)
        {
        $obj = $this->getDoctrine()->getRepository('App:Url')->findOne($uid);
        $label= $obj->getLabel();
        $linkref = new Linkref();
        $linkref->setLabel($label);
        $linkref->setPath($path);
    ...
    public function addLocation($pid,$lid)
    {
        ...
        if(!$lref)
        {
        $obj = $this->getDoctrine()->getRepository('App:Location')->findOne($lid);
        $label= $obj->getLabel();
        $linkref = new Linkref();
        $linkref->setLabel($label);
        $linkref->setPath($path);
    ...
    {
        $this->getDoctrine()->getRepository("App:Linkref")->deleteOne($lid);
        return $this->redirect("/admin/person/".$pid);
    }
     ...
     public function delete($pid)
    {
         ...
         $this->getDoctrine()->getRepository("App:Linkref")->deleteGroup("person",$pid);
         $this->getDoctrine()->getRepository("App:Imageref")->deleteGroup("person",$pid);

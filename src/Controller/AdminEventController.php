...
class AdminEventController extends AbstractController
{
     ...
     public function Editone($eid)
     {
        ...
        if(count($children))
        {
          ...
          // usort($children, function ($item1, $item2) {return ($item1['startdate'] <=> $item2['startdate']);});
           uasort($children,  array("App\Controller\EventController", 'datecmp'));
       ...
       // $linkrefs =$this->getDoctrine()->getRepository("App:Linkref")->findGroup('event',$eid);
          $linkrefs = $this->linkref->getLinks("event",$eid, $this->lang);
    ...
    {
        $url =  $this->getDoctrine()->getRepository("App:Url")->findOne($uid);
        $em = $this->getDoctrine()->getManager();
        $newp = new Linkref();
        $newp->setObjid((int)$eid);
    ...
    {
     $link = $this->getDoctrine()->getRepository("App:Linkref")->findOne($lid);
     $em = $this->getDoctrine()->getManager();
      $em->remove($link);
     $em->flush();
        return $this->redirect('/admin/event/'.$eid);
        ...
        foreach($events as $event)
            {
                ...
                $text_ar = $this->getDoctrine()->getRepository("App:Text")->findGroup("event",$eventid);

...
class ContentController  extends AbstractController
{
    ...
    public function __construct( MyLibrary $mylib ,RequestStack $request_stack, LinkrefController  $linkref, ParameterBagInterface $params )
    {
        $this->mylib = $mylib;
    ...
    {
        $def = $this->params->get('defaultcontent');
    ...
    public function ShowContentLang($sid,$lang)
    {
        ...
        $linkrefs = $this->linkref->getLinks("subject",$sid, $this->lang);
        $langlist = array();
    ...
    public function xShowcontent($cid,$lang)
    {
        ...
        $linkrefs = $this->linkref->getLinks("subject",$content->getSubjectid(), $this->lang);
        return $this->render('content/showone.html.twig',
        [
        'message' =>  '',
        'lang'=>$this->lang,
    ...
    public function Editcontent($sid)
    {
        ...
        {
            $text_ar =  $this->getDoctrine()->getRepository("App:Text")->findGroup('content',$content->getContentid());
            $title = $this->mylib->selectText($text_ar,'title',$this->lang);
            if($title)
        ...
        $linkrefs = $this->linkref->getLinksfrom("content",$sid);
    ...
    public function ShowNews()
    {
        ...
        $latest=null;
        $this->lang = $this->requestStack->getCurrentRequest()->getLocale();
        $contents = $this->getDoctrine()->getRepository("App:Content")->findNews();
        if (count($contents)<1)
        {
            $message = 'no.news';
        }
        else
        {
            ...
            {
              $content  =  $this->getDoctrine()->getRepository("App:Content")->findContentLang($subjectid,$this->lang);
              $text = $content->getText();
              $text = $this->insertInsertions($text);
              $content->setText( $text);
              $contents[] = $content;
    ...
    public function ContentSearch($search,Request $request)
    {
        ...
        {
            $sfield = "%".$pfield."%";
            $contents = $this->getDoctrine()->getRepository("App:Content")->findSearch($sfield);
    ...
    public function urlinsert($token_list)
    {
        ...
        $url =  $this->getDoctrine()->getRepository("App:Url")->findOne($urlid);
        if($url)
        {
            $label= $url->getLabel();
    ...
    {
        $osida = $this->getDoctrine()->getRepository("App:Content")->findMaxSid();
        $osidint = $osida[0][1];
        return $osidint + 1;
    }
}

...
use App\Form\Type\LinkrefForm;

use Symfony\Component\Routing\Annotation\Route;
use Symfony\Bundle\FrameworkBundle\Controller\AbstractController;
use Symfony\Component\HttpFoundation\RequestStack;

class LinkrefController extends AbstractController
{
    ...
    public function EditEventGroup($eid)
    {
        ...
        foreach( $refs as $ref)
        {
            $texts_ar[$ref['linkid']] =  $this->getDoctrine()->getRepository("App:Text")->findGroup('linkrefs',$ref['linkid']);
    ...
    public function AddAlink($otype1,$oid1,$otype2,$oid2)
    {
        // $obj = $this->getDoctrine()->getRepository('App:$otype2')->findOne($oid2);
        // $label= $obj->getLabel();
        $user = $this->getUser();
        $time = new \DateTime();
    ...
    {
        $this->lang = $this->requestStack->getCurrentRequest()->getLocale();
        $links = $this->getDoctrine()->getRepository("App:Linkref")->findGroup($objecttype,$objectid);
        $link_ar = array();
        foreach( $links as $link)
        {
            ...
            $linkref["id"]= $linkid;
            $label = $link['label'];
            $texts_ar =  $this->getDoctrine()->getRepository("App:Text")->findGroup('linkref', $linkid);
            if($texts_ar)
            {
            ...
            else
            {
                ...
                if($objtype =="content")
                {
                    $content = $this->getDoctrine()->getRepository("App:Content")->findContentLang( $objid,$this->lang);
                    $label = $content->getTitle();
                }
                else
                {
                $texts_ar =  $this->getDoctrine()->getRepository("App:Text")->findGroup($objtype, $objid);
                if($texts_ar)
                {
                  $label = $this->mylib->selectText($texts_ar,'title',$this->lang);
                }
    ...
    {
        $this->lang = $this->requestStack->getCurrentRequest()->getLocale();
        $links = $this->getDoctrine()->getRepository("App:Linkref")->findbyTarget($objecttype,$objectid);
